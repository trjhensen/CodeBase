function [bestCorrPerSizeList, minbestCorrPerSizeList, rxnsOfInterest] = clusterFluxRaCorrADRC(paths, top, onlyCorrTopMicrobes)
...
fluxValues.ID = erase(fluxValues.ID,{'mWBM_','_female','_male'});

% Reorder flux ids to microbiome ids
[~,ia,ib]=intersect(fluxValues.ID,relAbun.Row,'stable');
fluxValues = fluxValues(ia,:);
relAbun = relAbun(ib,:);
...
RHO = corr(rxnFlux, clusterAbundances,'type','Spearman'); % Spearman is still better so use it for now
