function [metadataPruned, metadataIntermedPruning] = pruneMetadataADRC(metadata)
...
sum(matches(string(metadata.NACCUDSD),'impaired non-mci'))
...
elseif 0
    isMCIorDementia = matches(cogData, {'AD dementia'});
    noAD = matches(metadata.AD, 'no_AD');
    metadata(isMCIorDementia & noAD, :) = [];
else
