function [sampleImportanceTable, explVar] = sampleOutliersInFluxesADRC(fluxPath, metadataPath, saveDir)
...
fluxTable.ID = erase(fluxTable.ID,{'mWBM_','_male','_female'});
...
f2 = figure;
% PD samples
idx = matches(metadata.(varOfInterest),'AD');
scatter3(PCscores(idx,1), PCscores(idx,2), PCscores(idx,3), 'MarkerEdgeColor',colorMap(1,:));
hold on;
% Controls
idx = matches(metadata.(varOfInterest),'no_AD');
scatter3(PCscores(idx,1), PCscores(idx,2), PCscores(idx,3), 'MarkerEdgeColor',colorMap(2,:));
% Sample ID labels
text(PCscores(:,1), PCscores(:,2),PCscores(:,3), fluxTable.ID,'FontSize',7)

% Axis titles
xlabel(strcat("PC1 explained variance: ",string(explVar(1)),"% "))
ylabel(strcat("PC2 explained variance: ",string(explVar(2)),"% "))
zlabel(strcat("PC3 explained variance: ",string(explVar(3)),"% "))
