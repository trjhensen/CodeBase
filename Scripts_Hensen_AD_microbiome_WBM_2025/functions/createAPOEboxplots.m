function [fig,t] = createAPOEboxplots(rxnsOfInterest, preparedInputTable, preparedMetadata, results_APOE, saveDir)
...
rxnsOfInterest(matches(rxnsOfInterest,'DM_dchac[bc]'))=[];

% Generate boxplot
fig = figure('Position',[147,80,1769,818]);
...
% Save figure 
% exportgraphics(fig,fullfile(saveDir,'flux_APOE_boxplots.png'),'Resolution',300)
end
...
% Create boxplot
plt = boxchart(group,values,'GroupByColor',group,'BoxWidth',0.8,'JitterOutliers','off');
fSize = 6;
ylabel("Flux in mmol/day/person","FontSize",fSize)
hold on
